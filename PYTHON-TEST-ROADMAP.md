...
| `william/tests/test_conditions.py` | Partial. CIWI now covers Python fixture shapes `co0`-`co4` and `dag0`-`dag3` using native composite literals and input placeholders. Remaining condition fixtures are `co5`-`co21` and `dag4`-`dag7`. | Composite conditions decide which inversions/search attachments are legal. This is high leverage for learned composites later. |
...
1. Composite/condition fixture expansion.
