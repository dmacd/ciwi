...
| `william/tests/test_alice_pipeline.py::TestMatrixRegressionDebugPipeline` | Active macro target. Direct matrix optimizer and graph-level `try_to_optimize` behavior are covered on deterministic `1000 x 10` fixtures. The supplied-solution single compression step is covered on an exact NumPy fixture through symbolic `Dot`/`Add` Wunderbaum search plus optimizer-backed `w` improvement. Next cover greedy matrix runs with and without the supplied solution. | This is the first major post-core end-to-end demonstration. It proves graph search can produce a symbolic structure whose permeable numeric leaves are optimized under the same DL objective Python uses. |
