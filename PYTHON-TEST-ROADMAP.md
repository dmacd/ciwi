...
| `william/tests/test_wunderbaum.py::test_wunderbaum_iteration` | Serial solution case covered. CIWI finds the native graph option expression `setitem(repeat(3, [45]), negate([-1 -2]), negate([-87 -87]))` with an injected registry and explicit declarations for Python's standalone operator set. The Python parallel-drain shape is also covered through `wunderbaum/iterate-parallel` with a bounded local worker search and `threshold-dl 0`. | This is the next end-to-end Wunderbaum case beyond Alice sequence compression, and it exercises a different operator family. |
...
| `william/tests/test_wunderbaum.py` optimizer helpers | Covered for `_extract_optimizables` and `_apply_opt_values` behavior. CIWI now checks scalar/small-array optimizer extraction, large-array skipping, and mixed scalar/float-array/integer-array writeback in `ciwi.graph-optimize-test`. | These are helper-level tests, but they protect optimizer-backed Wunderbaum/Alice candidate scoring. |
...
- Port the skipped Python debug rows as staged CIWI parity evidence. The
  `try_to_optimize`, single compression-step, and greedy single-factor rows are
  now covered together in `ciwi.iris-classification-test`; next is full Iris.
