...
| `william/tests/test_composite.py` | Partial. CIWI now covers native shared-DAG execution for `dag4`/`dag5`, `dag5` extra-branch inversion, exact `co2`/`co3`/`co4` inverse rows, `co0`/`co12` sequence inverses, `co7` nested `abs`, `trees13` `map(abs, x)` inverse branching and cap behavior, `trees0` concat inversion, `trees12`/`trees17` conversion-map inverses, `trees7` `bmap(add, ...)` inversion through `urange`, invalid `co10` `urange/insert`, `co11` `trange/repeat/listwrap/insert`, and the full `co0`-`co21` graph commutativity table. Remaining useful slices are inverse rows requiring `cumop`/`div`, richer `bmap`, `getitem`/`setitem` variants, or native DAG fixtures, then spec synchronization. Do not port S-expression reconstruction as parser parity. | Learned and builtin composites need to behave like native operators. This is central to the long-term library-compression plan. |
