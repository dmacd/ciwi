...
| --- | --- | --- |
| `william/tests/test_bottleneck.py::test_min_desc_len` | `section1`, `section2`, `section3`, and `section4` are covered with Clojure graph literals and Python DL constants. Next add `mult_negate_add` and `regression`; decide whether to add `mult_negate` as an empty-selection/raw bottleneck smoke test. | This is the best direct proof that CIWI's global shared-DAG minimizer matches WILLIAM outside Alice. |
...
1. Remaining bottleneck golden MDL cases: `mult_negate_add` and `regression`.
2. Propagation golden cases.
3. Delayed-builder exact materialization cases.
4. Wunderbaum `setitem/repeat/negate` solution case.
5. Composite/condition fixture expansion.
