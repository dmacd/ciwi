...
(def ^:private iris-random-state-0-permutation
...
(defn- iris-debug-fixture
  []
  (let [perm iris-random-state-0-permutation]
    {:target (dense/array (mapv #(quot % 50) perm))
     :factor (dense/array (mapv iris-sepal-length perm))
     :threshold 4.8}))
