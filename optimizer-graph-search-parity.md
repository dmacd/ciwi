...
| `test_alice.py::test_single_task_parallel` matrix row | Covered for matrix task | `ciwi.alice.matrix-regression-test/matrix-regression-parallel-greedy-without-solution-reaches-threshold` | Runs the exact matrix regression greedy row through CIWI's opt-in `:num-workers 8` Wunderbaum path with optimizer-backed candidates. This is completion parity for the matrix task, not a timing claim. |
...
solution hint are covered in the dedicated `ciwi.iris-classification-test`
namespace. These rows are useful optimizer-backed application evidence, but
they are still experimental and do not yet claim strong classifier quality.
CIWI embeds the canonical Iris sepal-length vector and Python
`RandomState(0)` permutation directly in that test namespace to avoid a
runtime sklearn dependency. The fixed graph test mirrors Python's
...
8. Done: wire optimizer-backed candidates into Alice/Wunderbaum and cover
   ...
   exact NumPy fixture. The matrix greedy row is also covered through the
   opt-in parallel `:num-workers 8` path.
