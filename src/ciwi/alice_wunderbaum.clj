...
(defn declarations-for-registry
  ...
  ([registry {:keys [operator-ids counts]
   ...
   (let [registry (require-registry registry)
         ...
         declarations-by-op (group-by :op alice-operator-declarations)
         table-order (distinct (map :op alice-operator-declarations))
       ...
       (mapv (fn [declaration]
               (let [k [(:op declaration)
                 ...
                 (assoc declaration
                        ...
                        :dl op-dl
                        :jitter (get python-alice-dl-jitter k 0.0))))
             declarations)))))
