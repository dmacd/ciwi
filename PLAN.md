...
- Current working tree has the first Python-scale core Wunderbaum parity rows,
  ...
  fixes; tests pass locally with 120 tests and 560
  assertions.
...
- Python-scale `simple_repeat`, `insert_repeat`, `repeat_with_noise`, and
  `simply_linear` pass through the injected Alice operator basis via
...
- `ciwi.alice-wunderbaum/run-greedy-task` now mirrors Python GreedyAlice's
  ...
  `insert_repeat`, and `simply_linear` now reach the same Python-shaped
  cumsum/insert solutions.
...
- Proceed to `insert_repeat2`, then `insert_repeat3`, keeping every fix tied to
...
- If runtime parity becomes the immediate priority, profile delayed
  ...
  paths on `simple_repeat` and `simply_linear`, where CIWI still has a modest
  warm-runtime gap despite matching compression behavior.
